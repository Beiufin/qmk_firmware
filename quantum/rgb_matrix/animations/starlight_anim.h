...
void set_starlight_color(int i, effect_params_t* params, rgb_config_t* config) {
...
bool STARLIGHT(effect_params_t* params, rgb_config_t* config) {
    if (!params->init) {
        if (scale16by8(g_rgb_timer, qadd8(config->speed, 5)) % 5 == 0) {
            int rand_led = rand() % RGB_MATRIX_LED_COUNT;
            set_starlight_color(rand_led, params, config);
        }
        return false;
    }
    ...
    for (int i = led_min; i < led_max; i++) {
        set_starlight_color(i, params, config);
    }
