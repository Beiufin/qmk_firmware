...
static void raindrops_set_color(int i, effect_params_t* params, rgb_config_t* config) {
    ...
    HSV hsv = {0, config->hsv.s, config->hsv.v};

    // Take the shortest path between hues
    int16_t deltaH = ((config->hsv.h + 180) % 360 - config->hsv.h) / 4;
    if (deltaH > 127) {
        deltaH -= 256;
    } else if (deltaH < -127) {
        deltaH += 256;
    ...
    hsv.h   = config->hsv.h + (deltaH * (random8() & 0x03));
...
bool RAINDROPS(effect_params_t* params, rgb_config_t* config) {
    ...
    if (!params->init) {
        ...
        if (scale16by8(g_rgb_timer, qadd8(config->speed, 16)) % 10 == 0) {
            raindrops_set_color(random8_max(RGB_MATRIX_LED_COUNT), params, config);
        }
    } else {
