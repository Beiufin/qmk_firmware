...
static bool PIXEL_FLOW(effect_params_t* params, rgb_config_t* config) {
    // LED state array
    ...
    inline uint32_t interval(void) {
        return 3000 / scale16by8(qadd8(config->speed, 16), 16);
    }
    ...
    if (params->init) {
        ...
        for (uint8_t j = 0; j < RGB_MATRIX_LED_COUNT; ++j) {
            led[j] = (random8() & 2) ? (RGB){0, 0, 0} : hsv_to_rgb((HSV){random8(), random8_min_max(127, 255), config->hsv.v});
        }
    }
    ...
    if (!rgb_matrix_check_finished_leds(led_max)) {
        ...
        // Fill last LED
        led[led_max - 1] = (random8() & 2) ? (RGB){0, 0, 0} : hsv_to_rgb((HSV){random8(), random8_min_max(127, 255), config->hsv.v});
        // Set pulse timer
